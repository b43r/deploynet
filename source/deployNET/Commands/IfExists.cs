...
{
    /// <summary>
    /// This command checks if a file or folder exists and executes a serie of other commands if yes.
    /// </summary>
    class IfExists : Command
    {
        ...
        /// <summary>
        /// Create a new ifexists command from an xml node.
        /// </summary>
        /// <param name="node">xml node</param>
        /// <param name="logger">reference to logger instance</param>
        public IfExists(XmlNode node, ILog logger)
            : base(node, logger)
        {
            if ((node.LocalName != "ifexists") ||
                (!node.HasAttribute("file") && !node.HasAttribute("folder")))
            {
                throw new ArgumentException("No valid <ifexists> node.");
            }
        ...
        /// <summary>
        /// Execute all child commands if the file or folder exists.
        /// </summary>
